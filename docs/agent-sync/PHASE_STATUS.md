...
- [x] Production proof rollout scaled to 2,000 unique actors with retry-aware status: 1,710 `fixture_replay_passed`, 290 `runtime_smoke_passed`, 0 `api_mapped`, 0 failures, 0 `live_e2e_passed`, 0 `ui_route_passed`; raw ledger rows 2,172 after transient transport retries.
- [x] Production proof rollout scaled to 3,000 unique actors with retry-aware status: 2,583 `fixture_replay_passed`, 417 `runtime_smoke_passed`, 0 `api_mapped`, 0 failures, 0 `live_e2e_passed`, 0 `ui_route_passed`; raw ledger rows 3,172 after transient transport retries.
- [x] Production proof rollout scaled to 4,000 unique actors with retry-aware status: 3,452 `fixture_replay_passed`, 548 `runtime_smoke_passed`, 0 `api_mapped`, 0 failures, 0 `live_e2e_passed`, 0 `ui_route_passed`; raw ledger rows 4,173 after transient transport retries.
- [x] Production proof rollout scaled to 5,000 unique actors with retry-aware status: 4,317 `fixture_replay_passed`, 683 `runtime_smoke_passed`, 0 `api_mapped`, 0 failures, 0 `live_e2e_passed`, 0 `ui_route_passed`; raw ledger rows 5,174 after transient transport retries.
- [x] Proof rollout loop remedy added: partial 6,000-wave ledger was discarded, the repo returned to the clean 5,000-actor checkpoint, and `scripts/run_actor_proof_factory.py` now supports `--max-runtime-seconds` for bounded checkpoint waves.
